...
def drop_all_rows():
	...
	cursor.execute("DELETE FROM PlayerIDs")
...
def post_create_org():
	...
	cursor.execute("INSERT INTO Organizations ( name, location, description, orgType, email, link ) VALUES ( %s, %s, %s, %s, %s, %s )", [data['org_name'], data['location'], data['description'], data['org_type'], data['email'], data['link']])
	cursor.execute("SELECT * FROM Organizations WHERE name = %s", [data["org_name"]])
...
#curl --request POST -H "Content-Type: application/json" -d '{"name":"bob bob","email":"bob@bob.com","password":"bob!"}' http://127.0.0.1:5000/CreateUser
@app.route('/setPlayerID', methods=['POST'])
@cross_origin(origin='*')
def post_set_player_id():
	data = request.get_json()
	cursor = mysql.connection.cursor()
	...
	cursor.execute("SELECT * FROM PlayerIDs WHERE user_id = %s", [userId])
	if cursor.rowcount > 0:
		retIdData = {}
		retIdData["message"] = "PLAYERID ALREADY EXISTS"
		return jsonify(retIdData)

	cursor.execute("INSERT INTO PlayerIDs ( player_id, user_id ) VALUES ( %s, %s )", [data['playerID'], userId])
	mysql.connection.commit()
	
	retIdData = {}
	retIdData["PlayerId"] = data['playerID']
	retIdData["UserId"] = userId
	return jsonify(retIdData), 200

@app.route('/deletePosting', methods=['POST'])
@cross_origin(origin='*')
def delete_posting():
	data = request.get_json()
	cursor = mysql.connection.cursor()
	...
	cursor.execute("DELETE FROM Applicants WHERE post_id = %s", [postId])
	cursor.execute("DELETE FROM Answers WHERE post_id = %s", [postId])
	cursor.execute("DELETE FROM Questions WHERE post_id = %s", [postId])
	cursor.execute("DELETE FROM Postings WHERE post_id = %s", [postId])
...
def update_applicant():
	...
	HEADER = {
		...
		'Authorization': "Basic " + "YzI4Y2FjYzEtZmZkZC00MmFhLTlkZWItMzk4ZTVkOGFlNWUw"
	}
	
	payload = {
		'app_id': "ee43c9de-4f3b-4710-bd29-56ddae58a30a"
	}
	
	cursor.execute("SELECT * FROM PlayerIDs WHERE user_id = %s", [user_id])
	results = cursor.fetchall()
	playerId = ""
	for row in results:
		playerId = row[0]
	...
	if playerId is "":
		retIdData = {}
		retIdData["message"] = "PLAYERID DOESN'T EXIST"
		return jsonify(retIdData)
	
	payload['contents'] = { 'en': "One of your application status' has been changed!" }
...
def get_all_org_info():
	...
	for row in results:
		...
		orgData["name"] = row[1]
		orgData["id"] = row[0]
		orgData["location"] = row[2]
		orgData["contactEmail"] = row[3]
		orgData["infoLink"] = row[4]
		orgData["orgType"] = row[5]
		orgData["members"] = []
